...
public class BinaryMinHeapTest {
   ...
   * impossible index);
   * 
   * I adopted solution 1 that I like as uses language constructs to handle corner cases; also the 2nd solution has some
   * appeal, as it turns a special case into a standard one, but I am afraid this could easily cause infinite loops;
  ...
  /**
   * Answering the data for an index out of bounds is not an easy answer. Possible answers are: 1. you are out of bounds
   * => throw exception; 2. I have no value for that index => return null, like accessing an existing, but empty
   * position;
   * 
   * I adopted solution 1 that I like as uses language constructs to handle corner cases; also the 2nd solution has some
   * appeal, but I prefer the solution from standard Java libs.
   */
