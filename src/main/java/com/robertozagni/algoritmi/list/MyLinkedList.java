...
public class MyLinkedList {
	...
	/**
	 * Crea una lista che ha come primo elemento il valore del nodo passato.
	 * La lista quindi contiene tutti i valori dei nodi collegati al nodo passato.
	 * Per ogni nodo della lista passata il costruttore crea un nuovo nodo privato
	 * con la referenza al valore del nodo passato.
	 * Questo esclude side effect sui puntatori tra nodi, 
	 * ma non sui valori contenuti nei nodi, che quindi sarebbe meglio 
	 * se fossero valori immutabili.
	 ...
	 * !!! ATTENZIONE ai SIDE EFFECT !!!
	 * 1) Questo metodo aggiunge alla lista i nodi di cui viene passata la referenza, per cui 
	 *    A - elementi della lista possono essere variati senza accedere alla lista,
	 *        rompendo il conteggio dei nodi;
	 *    B - elementi di cui si hanno riferimenti al di fuori della lista possono
	 ...
	 * 3) Se viene aggiunto un nodo che punta ad un nodo gi� nella lista 
	 ...
	 * 		   Se il nodo passato � null viene ritornato 0. 
	...
	/**
	 * Aggiunge alla lista il valore del nodo passato e dei nodi ad esso collegati,
	 * in modo sicuro e senza side effects (se non sui valori puntati dai nodi).
	 * Alla fine l'ultimo nodo della lista ha il valore dell'ultimo elemento della catena passata.
	 * Ritorna il numero di elementi aggiunti.
	 * @param start Il nodo da cui iniziare ad aggiungere una catena.
	 * @return Il numero di nodi della catena appena aggiunta, incluso quello passato.
	 * 		   Se il nodo passato � null viene ritornato 0. 
	...
	public static int countNodesRecursive(Node start) {
		...
		//Node current = start;
		if (start != null) {
			if (DEBUG) System.out.print("=>"+start.getValue());
	...
	/**
	 * Una classe che rappresenta un Nodo della MyLinkedList.
	 * Ha un valore di tipo <T> ed un nodo seguente (next). 
	 * @author Roberto
	 *
	 */
	public static class Node<T> {
		 ...
		 *        Pu� essere null se non si vuole collegare un nodo seguente.
		 */
		 ...
		 * Vale null se non ci sono altri nodi, quindi quello corrente � l'ultimo nodo.
		 */
		private Node next = null;
		
		/**
		 * Ritorna il nodo collegato al nodo attuale, cio� il prossimo nodo della lista.
		...
		/**
		 * Connette il nodo corrente al prossimo nodo della lista.
		 * Non ci sono controlli sul tipo di valore collegato al nodo passato.
